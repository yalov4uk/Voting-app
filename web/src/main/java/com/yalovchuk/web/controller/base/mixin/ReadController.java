...
public interface ReadController<T extends Bean<K>, K extends Number, D extends BeanDto<T, K>,
        R extends BeanResource<T, K>> extends GetService<T, K, D, R> {

    @RequestMapping(value = "/{id}", method = RequestMethod.GET)
    default HttpEntity<R> read(@PathVariable K id) {
        return new ResponseEntity<>(getService().readResource(id), HttpStatus.OK);
    }
}
