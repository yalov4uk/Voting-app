...
public interface UpdateController<T extends Bean<K>, K extends Number, D extends BeanDto<T, K>,
    ...
    default HttpEntity<R> update(@PathVariable K id, @RequestBody D beanDto) {
        return new ResponseEntity<>(getService().updateResource(id, beanDto), HttpStatus.OK);
    }
}
